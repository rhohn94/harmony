...
<!-- Project agent: this file is the per-project UX design-language authority.
     ...
     the `grm-design-language-adapt` skill to pull and adapt the upstream source.
...
<!-- Project agent: record whether this project tracks `upstream` or runs in
     strict-`local` mode (see the `source:` front-matter field above).
       - `upstream`: `grm-design-language-adapt` clones the `source-url` repo, freezes
     ...
     See the `grm-design-language-adapt` skill for the full behaviour of each mode. -->
...
<!-- Project agent: list which upstream components this project adapts, and to
     ...
     the `grm-design-language-adapt` skill). -->
...
machine-addressable companions: `grm-ux-demo-build` reads `components.md` for which
...
<!-- Project agent: this is the project-specific contract. Each item asserts "the
     ...
     `grm-ux-demo-build` skill produces the demo these items are reviewed against. -->
