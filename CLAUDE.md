...
the **`stealth-mode-switch`** skill; only the active state's content sits between
...
has no equivalent). Add new ones with the **`workflow-scaffold`** skill.
...
`docs/integration-workflow.md` §Workflow-based-orchestration.
...
with **`design-language-adapt`**; verify with **`ux-demo-build`** (opt-in).
